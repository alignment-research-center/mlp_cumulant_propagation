...
def test_factored_kprop_augment_not_implemented():
    ...
    WK = linear_kprop(K, torch.randn(n, n) * math.sqrt(2 / n), k_max=3, set_metric=2. * torch.ones(n))
    with pytest.raises(NotImplementedError):
        factored_nonlin_kprop_k3(K_in=WK, nonlin_wick_coef=relu_wick_coef, augment=True)
